    ...
    <div className="space-y-6 p-4 sm:p-6">
      ...
      <Card>
        ...
        <CardContent>
          ...
          <tbody className="divide-y" key={`${filterTeam}-${statusFilter}-${searchQuery}`}>
                {loadingTasks ? (
                  ...
                  displayTasks.map((t, index) => (
                    ...
                    >
                      <td className="px-4 py-3 cursor-pointer" onClick={() => setDetailsTaskId(t.id)}>
                        <div className="flex items-center gap-2">
                          {t.project_number && <Badge variant="outline" className="h-5 px-1.5 text-[10px] font-bold bg-muted/50">{t.project_number}</Badge>}
                          <div className="font-medium text-slate-900 hover:underline">{t.client}</div>
                        </div>
                      ...
                      <td className="px-4 py-3 text-right">
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            <DropdownMenuItem onClick={() => setDetailsTaskId(t.id)}>
                              <Info className="mr-2 h-4 w-4" /> Voir les détails
                            </DropdownMenuItem>
          ...
          <TaskDetailsSheet taskId={detailsTaskId} onOpenChange={(o) => !o && setDetailsTaskId(null)} />
        </CardContent>
      </Card>
    </div>
  );
