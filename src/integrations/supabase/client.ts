...
if (!isConfigured) {
  if (typeof window !== 'undefined') {
    console.warn("⚠️ SUPABASE NOT CONFIGURED ! Vérifiez votre fichier .env (VITE_SUPABASE_URL et VITE_SUPABASE_PUBLISHABLE_KEY ou VITE_SUPABASE_ANON_KEY).");
    console.log("Diagnostic:", {
      SUPABASE_URL: SUPABASE_URL,
      SUPABASE_PUBLISHABLE_KEY: SUPABASE_PUBLISHABLE_KEY.length > 0 ? "*** (masquee)" : "VIDE",
      isConfigured
    });
  } else {
...
// Log supplémentaire pour le diagnostic dans Docker
if (!isConfigured) {
...
export const supabase = createClient<Database>(supabaseUrl, supabaseKey, {
  auth: {
    ...
    flowType: 'pkce',
    storageKey: 'sb-verdant-auth-token',
    // Évite LockAcquireTimeoutError (React Strict Mode / onglets multiples en local)
    lockAcquireTimeout: 10000,
    lock: async (_name, _acquireTimeout, fn) => fn(),
  },
  // Ajout d'un timeout global pour éviter les requêtes bloquantes
  db: {
    schema: 'public'
  }
});
