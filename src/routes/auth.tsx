...
import { Leaf, Loader2, Mail, UserPlus, User } from "lucide-react";
      ...
      <CardContent>
        {localMode && (
          <p className="mb-3 rounded-md border border-amber-500/40 bg-amber-500/10 px-3 py-2 text-xs text-amber-800 dark:text-amber-200">
            Supabase local : les comptes cloud ne sont pas copiés. Créez un compte via l’onglet Inscription (vous pouvez réutiliser le même e-mail et mot de passe).
          </p>
        )}
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as 'signin' | 'signup')}>
          ...
          <TabsContent value="signin"><SignInForm onNeedSignup={() => setActiveTab("signup")} /></TabsContent>
...
function SignInForm({ onNeedSignup }: { onNeedSignup: () => void }) {
  ...
  const getEffectiveEmail = (input: string) => {
    const trimmed = input.trim();
    if (trimmed.includes("@")) {
      return trimmed;
    }
    return `${nameToEmail(trimmed)}@verdura.fr`;
  };

  const handleResetPassword = async () => {
    if (!nameOrEmail) {
      toast.error("Veuillez saisir votre nom complet ou email pour réinitialiser votre mot de passe.");
      return;
    }
    const targetEmail = getEffectiveEmail(nameOrEmail);
    setResetBusy(true);
    try {
      const { error } = await authWithTimeout(supabase.auth.resetPasswordForEmail(targetEmail, {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    const targetEmail = getEffectiveEmail(nameOrEmail);
    console.log("Tentative de connexion pour:", targetEmail);
    try {
      if (!supabase) {
        toast.error("Le client Supabase n'est pas initialisé. Vérifiez vos variables d'environnement.");
        return;
      }
      const { error } = await authWithTimeout(supabase.auth.signInWithPassword({ email: targetEmail, password }));
      if (error) {
        console.error("Détails erreur connexion:", error);
        if (error.message === "Invalid login credentials" && isLocalSupabase()) {
          toast.error("Compte introuvable en local. Utilisez l’onglet Inscription pour le recréer.");
          onNeedSignup();
        } else {
...
function SignUpForm() {
  ...
  const [role, setRole] = useState<"agent" | "coordinator" | "admin">("admin");
  const [busy, setBusy] = useState(false);
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    const targetEmail = trimmedEmail.includes("@")
      ? trimmedEmail
      : `${nameToEmail(name)}@verdura.fr`;
    console.log("Tentative d'inscription pour:", targetEmail, "Rôle:", role);
    try {
      ...
      if (error) {
        ...
        toast.success(isLocalSupabase()
          ? "Compte local créé. Vous pouvez vous connecter."
  ...
  return (
    <form onSubmit={submit} className="space-y-3 pt-3">
      <div><Label>Nom complet</Label><Input placeholder="Ex: Jean Dupont" required value={name} onChange={(e) => setName(e.target.value)} /></div>
      <div><Label>Email</Label><Input type="email" placeholder="Ex: jeanmichel@digipous.com" required value={email} onChange={(e) => setEmail(e.target.value)} /></div>
